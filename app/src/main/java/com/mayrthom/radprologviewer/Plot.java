...
public class Plot extends Fragment {
    ...
    enum Unit {CPS, CPM, SIEVERT}
    Unit unit = Unit.CPM;
    Datalog datalog;
    int lineColor, backcolor, textcolor;
    ...
    @Override
        ...
        sharedViewModel.getDatalog().observe(getViewLifecycleOwner(), d -> {
            if (d != null) {
                datalog = d;
                setData();
            }});
    ...
    public void onResume() {
        super.onResume();
            ((AppCompatActivity) getActivity()).getSupportActionBar().setTitle("Datalog Plot");
    ...
    private void setData() {
        List<Entry> entries = toggleUnit(datalog.getEntrySet(),unit);
        ...
        if(entries != null && !entries.isEmpty()) {
            ...
            xAxis.setValueFormatter(new ValueFormatter() {
                ...
                public String getFormattedValue(float value) {
                    Date date = new Date( ( (long) value + datalog.getStartPoint())*1000);
    ...
    float getMaxVal(List<Entry> entries)
    {
    ...
    private List<Entry> toggleUnit(List<Entry> entries, Unit unit)
    {
        switch(unit)
        {
            ...
            case SIEVERT:
                return entries.stream()
                        .map(entry -> new Entry(entry.getX(), entry.getY() * (60)/datalog.getConversionFactor() ))
                        .collect(Collectors.toList());
            case CPS:
            default:
                return entries;
